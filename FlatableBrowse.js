...
(() => {
  ...
  const injectCss = () => {
    ...
    const css = [
      ...
      'height:calc(100vh - var(--lf-search-overlay-map-top,140px) - 12px)!important;',
  ...
  // === MAP FADE-IN (GSAP) ===
  // Prevents the grey MapLibre canvas from flashing before tiles arrive.
  // hideMapForFadeIn() runs at boot so the aside is opacity:0 immediately;
  // fadeMapIn() runs from map.on('load') when tiles + style are ready.
  // Defensive no-ops if GSAP is somehow missing.
  ...
  const boot = async () => {
    ...
    // before MapLibre's 'load' event fires.
    hideMapForFadeIn();
    ...
    map.on('load', () => {
      ...
      // Map tiles are ready — fade the aside in.
      fadeMapIn();
